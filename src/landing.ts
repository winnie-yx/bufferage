...
type LandingElements = {
  canvas: HTMLCanvasElement;
  enterButton: HTMLButtonElement;
  overlay: HTMLElement;
  shellFrame: HTMLElement;
};
...
export function initLandingExperience({
  ...
  const triggerEnter = () => {
    ...
    dust.forEach((particle, index) => {
      const angle =
        Math.atan2(particle.y - state.viewportHeight * 0.5, particle.x - state.viewportWidth * 0.5) +
...
function createTitleParticles(viewportWidth: number, viewportHeight: number): TitleField {
  ...
  const letterZones: TitleField["letterZones"] = [];
...
function createDustParticles(viewportWidth: number, viewportHeight: number) {
...
function createBurstParticles(viewportWidth: number, viewportHeight: number) {
  return Array.from({ length: 560 }, (_, index) => {
    const x = Math.random() * viewportWidth;
    const y = Math.random() * viewportHeight;
    const angle =
      Math.atan2(y - viewportHeight * 0.5, x - viewportWidth * 0.5) + (Math.random() - 0.5) * 0.8;
...
function drawTitleParticles(
  context: CanvasRenderingContext2D,
  particles: Particle[],
  pointer: { dx: number; dy: number; isInside: boolean; x: number; y: number },
  state: {
  ...
  particles.forEach((particle) => {
    ...
    const alphaFade =
      pointerOnLetter && pointerDistance < mouseRadius
