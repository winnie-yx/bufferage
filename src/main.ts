...
app.innerHTML = `
  <section class="landing-overlay">
    ...
    <div class="landing-content">
      ...
      <h1 class="landing-title">Hyperhesitation</h1>
...
const showcaseTitleCanvas = app.querySelector<HTMLCanvasElement>(".showcase-title-canvas");
const modelCanvas = app.querySelector<HTMLCanvasElement>(".model-canvas");
const enterButton = app.querySelector<HTMLButtonElement>(".landing-enter");
const landingCanvas = app.querySelector<HTMLCanvasElement>(".landing-canvas");
const landingOverlay = app.querySelector<HTMLElement>(".landing-overlay");
const shell = app.querySelector<HTMLElement>(".shell");
const shellFrame = app.querySelector<HTMLElement>(".shell-frame");
const modelPage = app.querySelector<HTMLElement>(".model-page");
const screenShell = app.querySelector<HTMLElement>(".computer-screen");
const questionCard = app.querySelector<HTMLElement>(".question-card");
const timerValue = app.querySelector<HTMLElement>(".timer-board__value");
const choiceButtons = Array.from(app.querySelectorAll<HTMLButtonElement>(".decision-button"));
const returnButton = app.querySelector<HTMLButtonElement>(".decision-return");
...
const resultScreen = app.querySelector<HTMLElement>(".question-screen--result");
const resultChoiceBadge = app.querySelector<HTMLElement>(".result-choice-badge");
const resultTimeBadge = app.querySelector<HTMLElement>(".result-time-badge");
const resultSummaryPercent = app.querySelector<HTMLElement>(".result-summary__percent");
const resultSummaryCount = app.querySelector<HTMLElement>(".result-summary__count");
...
let loaderFrame = 0;
let selectedChoice: "TRUE" | "FALSE" | null = null;
...
let responseStatsMemory: ResponseStats = {
  FALSE: 0,
  TRUE: 0,
};
let selectedPosterId = "03";

const updateQuestionEyebrows = (posterId: string) => {
...
const readResponseStats = (): ResponseStats => {
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<ResponseStats>;
...
const writeResponseStats = (stats: ResponseStats) => {
...
const renderTimer = (elapsedMs: number) => {
...
const randomAiChoice = (): Choice => (Math.random() > 0.5 ? "TRUE" : "FALSE");

const renderResult = (choice: Choice) => {
...
const startBootLoader = (onComplete: () => void) => {
  if (loaderFrame) {
    window.cancelAnimationFrame(loaderFrame);
  }
