...
export function initShowcaseTitleParticles({ canvas, interactionTarget }: ShowcaseTitleElements) {
...
function createTitleParticles(viewportWidth: number, viewportHeight: number) {
...
function drawShadowTitle(context: CanvasRenderingContext2D, viewportWidth: number, viewportHeight: number) {
  const fontSize = Math.min(viewportWidth * 0.27, 430);
...
function drawTitleParticles(
  ...
  particles.forEach((particle) => {
    ...
    const alphaFade =
      pointerOnTitle && pointerDistance < mouseRadius
