...
const posterImageConfigs: Partial<Record<string, PosterImageConfig>> = {
...
export function initPortfolioShowcase({ canvas, onSelect }: ShowcaseElements) {
  ...
  const posterItems: PosterItem[] = posters.map((poster, index) => {
    let posterTexture: any;
    ...
    const plane = new THREE.Mesh(
      new THREE.PlaneGeometry(poster.size[0], poster.size[1]),
      material,
    );
    const glow = new THREE.Mesh(
      new THREE.PlaneGeometry(poster.size[0] * 1.12, poster.size[1] * 1.12),
      new THREE.MeshBasicMaterial({
  ...
  const setHoveredItem = (nextId: number) => {
    if (state.hoveredId === nextId) return;
  ...
  const pickPosterAtPointer = () => {
    ...
    const intersections = raycaster.intersectObjects(
      posterItems.map((item) => item.plane),
      false,
    );
  ...
  renderer.setAnimationLoop((time: number) => {
    state.rotationVelocityY = THREE.MathUtils.lerp(
      state.rotationVelocityY,
      state.targetVelocityY + state.blankBoost,
      0.045,
    );
    state.rotationVelocityX = THREE.MathUtils.lerp(
      state.rotationVelocityX,
...
function createPosterTexture(poster: PosterDefinition, onUpdate?: () => void) {
